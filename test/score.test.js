...
import reducer, * as score from '../src/stores/score'
...
test('reducer', (assert) => {
  ...
  // assert.truthy(reducer(0, score.addPieceScore(10)) > pieceScore, 'piece score is more when level is higher')
  ...
  // assert.truthy(pointsFrom4Lines > pointsFrom1Line, 'Number of lines affects score')
...
test('selector', (assert) => {
  // assert.is(score.getScore({score: 123}), 123, 'getScore() returns the correct value')
})
